...
int ploadPin = 11;       // Connects to Parallel load pin the 165 leftmost 1st
int clockEnablePin = 8;  // Connects to Clock Enable pin the 165 shift register ke upar                 led -ground - arduino -12
int dataPin = 9;         // Connects to the Q7 pin the 165 rightmost
int clockPin = 10;       // Connects to the Clock pin the 165 leftmost 2
...
void loop() {
  ...
  if(millis()-timer>=180000){
    if(black_left>white_left){
      winning_glow(2);
    }
    else if(white_left>black_left){
      winning_glow(3);
    }
    else{
      winning_glow(1);
  ...
  else if(black_left==0 || white_left==0){
    if (black_left==0){
      winning_glow(3);
    }
    else{
